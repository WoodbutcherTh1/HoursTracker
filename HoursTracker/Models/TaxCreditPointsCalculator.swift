...
enum TaxCreditPointsCalculator {
    /// Base points for an Israeli resident.
    static let baseResidentPoints: Double = 2.25

    /// Approximate monthly value of one credit point (₪), 2025 baseline.
    static let creditPointMonthlyValueILS: Double = 242

    /// Average working days used to spread monthly credits/deductions onto a single day.
    static let averageWorkingDaysPerMonth: Double = 21.67

    static func creditPoints(for settings: WorkplaceSettings) -> Double {
    ...
    static func monthlyCreditValue(for settings: WorkplaceSettings) -> Double {
        creditPoints(for: settings) * creditPointMonthlyValueILS
    }

    static func dailyCreditValue(for settings: WorkplaceSettings) -> Double {
