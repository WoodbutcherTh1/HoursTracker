...
- **Merge strategy**: last-write-wins per record using `modifiedAt` (`WorkSession.touch()` bumps it on every edit). Sessions are merged by UUID; the newer copy wins. **Tombstones** (`SessionTombstoneStore` → `session_tombstones.json`) record locally deleted session IDs so a deleted shift cannot be re-imported from another device; full sync also deletes still-present remote tombstoned records. Tombstones older than 90 days are pruned.
- **CloudKit writes**: uploads/deletes run through `CloudWriteQueue` (actor) so Tasks cannot interleave. Saves use `CKModifyRecordsOperation` with `.changedKeys`; `serverRecordChanged` is resolved with newer-`modifiedAt`-wins while preserving the server change tag.
- **CloudKit entitlements**: personal-team builds keep `HoursTracker.entitlements` empty. Paid-team CloudKit builds must copy keys from `docs/HoursTracker.entitlements.cloudkit.example` (container `iCloud.com.hourstracker.app`) and set `HTCloudKitEnabled`.
...
- Requires an iCloud-signed-in device for sync (container `iCloud.com.hourstracker.app`).
- CloudKit is **compile-time opt-in** via `HTCloudKitEnabled` in Info.plist **and** a **runtime user toggle** (`iCloudSyncEnabled`, default off). Personal-team installs leave CloudKit off (empty entitlements) so `CKContainer` never initializes. For paid-team builds, copy `docs/HoursTracker.entitlements.cloudkit.example` into `HoursTracker.entitlements`, set `HTCloudKitEnabled`, and provision container `iCloud.com.hourstracker.app`.
