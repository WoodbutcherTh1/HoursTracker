...
- Work through Part A in order (it is priority-ordered), then Part B, respecting the guardrails in Part C. Verify with Part D.
...
## Part B — General enterprise hardening (apply with your own judgment)
...
## Part D — Verification you must run before finishing

1. `xcodegen generate` succeeds and `xcodebuild test` passes on an iPhone simulator, including your new tests (file protection, tombstone merge, CSV injection, settings clamping, temp-file cleanup, ID keychain migration).
2. Manual flows on simulator/device:
   - Fresh install → set worker info incl. ID → relaunch → data intact; inspect the app container: `workplace_settings.json` contains no plaintext ID; files carry the expected protection attribute.
   - Export PDF/TXT/DOCX/MD → share sheet works → after dismissal/relaunch the temp exports directory is empty.
   - "Delete All My Data" → sessions/settings/log/Keychain/temp files all gone; notifications cleared; (CloudKit build) records deleted.
   - Arrival reminders: enable → staged permission prompts appear → geofence entry fires a notification **without** the `location` background mode; denial states render correctly.
   - App Lock enabled → lock on launch and on return from background; privacy overlay covers content in the app switcher regardless.
   - Copy a shift row → pasteboard item is local-only and expires.
   - Import a >50 MB file and a 200-page PDF → clean localized error / bounded processing, no hang.
3. Grep gates: no `privacy: .public` on identifiers/errors; no raw `Data.write` outside the protected-write helper; no coordinates in any `ActivityLogStore.log` call.
4. Re-read `PrivacyInfo.xcprivacy`, `docs/PRIVACY.md`, and the in-app policy strings side by side — they must all describe identical behavior.

Deliver the work as a series of small, logically separated commits (one area per commit), each with tests, so the diff is reviewable.
