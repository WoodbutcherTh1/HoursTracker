...
> **Status:** implemented — unpaid breaks (per-session, with an auto-applied default for 6h+ shifts), rest-day/holiday rates (150%/175%/200%, auto-tagged from the configured weekly rest day), night shifts (7h standard day, auto-detected at ≥ 2h between 22:00–06:00), multiple sessions per day now share one daily overtime/gas allowance, and pay display uses a configurable currency. Separately delivered by the payroll/OCR branch: net-pay estimation with credit points and custom payroll months. Still open: weekly overtime caps and a bundled holiday calendar (holidays are a manual per-session flag today). Rest-day is workplace-driven (e.g. a שומר שבת employer closes for everyone regardless of individual worker religion), so the existing single configurable weekly rest day already covers it — no per-worker religion tagging needed.
