#!/usr/bin/env bash
# Scenario 4 × 5 + Scenario 5 × 1 after phone reachability re-push fix.
# Pass condition is polled observation of Documents JSON (test harness only);
# production sync path is event-driven (reachability / activation).
set -euo pipefail
...
mkdir -p "$RESULT_DIR"
REPORT="$RESULT_DIR/MANUAL_MATRIX_4x5_5x1.md"
...
{
  echo "# Scenario 4 × 5 + Scenario 5 × 1 (reachability re-push)"
  echo
